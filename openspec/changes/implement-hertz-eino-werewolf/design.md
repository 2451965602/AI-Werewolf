...
- AI 输出必须经过 domain 校验；调用失败或解析失败时由 application 有限重试后启发式回退，成功响应但目标非法时拒绝该动作且不变更游戏状态。
...
- AI 调用失败：
  - 第一步重试（有限次数）
  - 第二步启发式回退（基于规则和最小策略）
- AI 成功响应但目标非法：拒绝该动作，不执行 fallback mutation。
...
- 风险：AI 输出不稳定导致流程抖动
- 缓解：结构化输出约束 + 解析校验；调用/解析失败走重试与启发式回退，非法目标直接拒绝且不变更状态
